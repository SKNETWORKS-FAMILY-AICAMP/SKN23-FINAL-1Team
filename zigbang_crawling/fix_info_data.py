...
def fix_info_data():
    ...
    for file in sorted(csv_files):
        try:
            ...
            # 필요한 컬럼만 읽어서 메모리 절약
            df = pd.read_csv(file, usecols=['item_id', 'parking_available_text', "deposit", "rent"], low_memory=False)
            
            for _, row in df.iterrows():
                if row["status"] == "ACTIVE":
                    item_id = row['item_id']
                    text_val = str(row['parking_available_text'])
                    deposit = int(row["deposit"]) if row["deposit"] else 0
                    rent = int(row["rent"]) if row["rent"] else 0
                    # '주차 가능' 문구가 포함되어 있으면 True
                    is_parking = True if '주차 가능' in text_val else False
                    repairing_map[item_id] = (is_parking, deposit, rent) # 중복 ID는 마지막에 읽은 놈으로 덮어씌워짐
    ...
    # 3. 딕셔너리를 데이터프레임으로 변환
    update_df = pd.DataFrame([(list(repairing_map.keys()) + list(*repairing_map.values()))], columns=['item_id', 'has_parking', 'deposit', 'rent'])
    ...
    try:
        with engine.begin() as conn:
            ...
            result = conn.execute(sql_item_features)
            ...
            result = conn.execute(sql_items)
            print(f"성공! {result.rowcount}개의 매물 정보가 갱신됐어. 😤")
